...
def trajectory_plt(model, inputs, targets, timesteps, highlight_inputs=True,
                   include_arrow=False, save_fig=''):
    """Plots trajectory of input points when evolved through model. Works for 2
    and 3 dimensions.
...
def trajectory_plt2(model, inputs, targets, timesteps, highlight_inputs=False,
                   include_arrow=False, save_fig=''):
    """Plots trajectory of input points when evolved through model. Works for 2
    and 3 dimensions.
...
#get features for discrete model
def get_features(model, test_inputs):
    if model.augment_dim > 0:
        if model.is_img:
            # Add augmentation
            batch_size, channels, height, width = test_inputs.shape
            aug = torch.zeros(batch_size, model.augment_dim,
                              height, width).to(model.device)
            # Shape (batch_size, channels + augment_dim, height, width)
            test_inputs = torch.cat([test_inputs, aug], 1)
        else:
            # Add augmentation
            aug = torch.zeros(test_inputs.shape[0], model.augment_dim).to(model.device)
            # Shape (batch_size, data_dim + augment_dim)
            test_inputs = torch.cat([test_inputs, aug], 1)

    aug = torch.zeros(test_inputs.shape[0], test_inputs.shape[1]).to(model.device)
    x_aug = [torch.cat([test_inputs, aug], 1)]
    features = model.hamil_blocks(x_aug)
    return features


def trajectory_plt_discrete(model, inputs, targets, highlight_inputs=True,
                            include_arrow=False):
    import numpy
    import matplotlib.pyplot as plt
    import torch
    ...
    from mpl_toolkits.mplot3d import Axes3D, proj3d
    if model.augment_dim == 1:
        input_dim = 2 + model.augment_dim
    else:
        input_dim = 2
    alpha = 0.5
    color = ['red' if targets[i, 0] > 0.0 else 'blue' for i in range(len(targets))]
    features = get_features(model, inputs)
    features = numpy.asarray([a_.detach().numpy() for a_ in features])
    ...
    if input_dim == 2:
        ...
        plt.scatter(inputs[:, 0].numpy(), inputs[:, 1].numpy(), c=color,
                    alpha=alpha, linewidths=input_linewidths, edgecolor='orange')
        plt.scatter(features[-1, :, 0], features[-1, :, 1], c=color,
                    alpha=alpha, linewidths=0)
        ...
        ax.scatter(inputs[:, 0].numpy(), inputs[:, 1].numpy(),
                   inputs[:, 2].numpy(), c=color, alpha=alpha,
                   linewidths=input_linewidths, edgecolor='orange')
        ax.scatter(features[-1, :, 0], features[-1, :, 1],
                   features[-1, :, 2], c=color, alpha=alpha, linewidths=0)
...
def Hamiltonian_computation(model,inputt,targets=None,activation='relu',ponential='linear',show=False,multi=True,steps=10,y=10):
    ...
    if model.augment_dim>0:        
        aug = torch.zeros(inputt.shape[0], model.augment_dim)
        inputt = torch.cat([inputt, aug],1)

    z = torch.zeros(inputt.shape[0],inputt.shape[1])

    inputt = torch.cat([inputt, z],1)

            
    outs = odeint(model.odeblock.odefunc, inputt, integration_time,
                         rtol=model.tol, atol=model.tol, method=model.odeblock.method,
                         options={'max_num_steps': 1000})

    H=numpy.zeros((inputt.shape[0],steps))
    Ki=numpy.zeros((inputt.shape[0],steps))
    Po=numpy.zeros((inputt.shape[0],steps))
    W=model.odeblock.odefunc.fc2.weight.detach().numpy()
    K=model.odeblock.odefunc.fc1.weight.detach().numpy()
    b=model.odeblock.odefunc.fc1.bias.detach().numpy()
    for i in range(steps):
        x=outs[i,:,:]
        z_T=x[:,model.data_dim+model.augment_dim:x.shape[1]].detach().numpy()
        x_T=x[:, 0:model.data_dim+model.augment_dim]
        enegy_kinetic=1/2*numpy.sum(numpy.multiply(numpy.dot(z_T,numpy.dot(W.T,W)),z_T),1)
        if ponential== 'quadratic':
            x_T=x_T.detach().numpy()
            out= numpy.sum(numpy.multiply(numpy.dot(x_T,K),x_T),1)+numpy.dot(x_T,b)
            if activation == 'relu':
                out=numpy.maximum(out, 0)
                enegy_potential= 1/2*numpy.power(out,2)
            elif activation == 'tanh':
                enegy_potential=numpy.log(abs(numpy.cosh(out)))
        elif ponential== 'linear':
            out=model.odeblock.odefunc.fc1(x_T).detach().numpy()
            if activation == 'relu':
                out=numpy.maximum(out, 0)
                enegy_potential= numpy.sum(1/2*numpy.power(out,2),1)
            elif activation == 'tanh':
                enegy_potential= numpy.sum(numpy.log(abs(numpy.cosh(out))),1)
        H[:,i]=enegy_kinetic+enegy_potential
        Po[:,i]=enegy_potential
        Ki[:,i]=enegy_kinetic
    if show==True:
        colors=[]
        colors= ['red' if targets[i] > 0.0 else 'blue' for i in range(targets.shape[0])]
        ...
        plt.subplot(1, 3, 2)
        for i in range(targets.shape[0]):
            plt.plot(Po[i,:],color =colors[i])
        plt.xlabel('TimeSteps')
        plt.ylabel('potential')
        plt.ylim(0,y)
        plt.subplot(1, 3, 3)
        for i in range(targets.shape[0]):
            plt.plot(Ki[i,:],color =colors[i])
        plt.xlabel('TimeSteps')
        plt.ylabel('kinetic')
        plt.ylim(0,y)
        return
...
def hamiltonian_plot(model, plot_range=(-2., 2.), num_steps=201,activation='relu',ponential='linear',step=0,enegy=0):
    import numpy as np
    from matplotlib.colors import LinearSegmentedColormap
    from matplotlib.patches import FancyArrowPatch
    from mpl_toolkits.mplot3d import Axes3D, proj3d
    ...
    hamiltonians=Hamiltonian_computation(model,grid ,targets=None ,activation=activation,ponential=ponential,show=False)[enegy][:,step]
    vmax=np.mean(hamiltonians)
    vmin=min(hamiltonians)
    
    pred_grid = hamiltonians.reshape(num_steps, num_steps)

    # Set up a custom color map where -1 is mapped to blue and 1 to red
    colors = [ (1, 1, 0), (1, 0.5, 0),   (1, 0, 0)]
    colormap = LinearSegmentedColormap.from_list('cmap_red_blue', colors, N=15)


    plt.imshow(pred_grid, vmin=vmin, vmax=vmax, cmap=colormap, alpha=0.75)
    plt.colorbar()
    plt.xlabel('Hamiltonian of %s active %s ponential at time t[%d]'% (activation,ponential,step))
    plt.tick_params(axis='both', which='both', bottom=False, top=False,
                        labelbottom=False, right=False, left=False,
                        labelleft=False)
    return hamiltonians
...
def hamiltonian_hist(model, plot_range=(-2., 2.), num_steps=201,activation='relu',ponential='linear',step=0,enegy=0):
    import numpy as np
    # Grid the input space
    grid = torch.zeros((num_steps * num_steps, 2))
    idx = 0
    ...
    hamiltonians=Hamiltonian_computation(model,grid ,targets=None ,activation=activation,ponential=ponential,show=False)[enegy][:,step]
    plt.hist(hamiltonians,color='red',bins=40,range=(0,4))


def Hamiltonian_discret_computation(model, inputs, targets=None, show=False, multi=True, y=10):
    import numpy
    import matplotlib.pyplot as plt
    if multi == False:
        inputs = inputt.view(1, inputs.shape[0])
    features = get_features(model, inputs)
    activation = model.activation

    H = numpy.zeros((inputs.shape[0], int(model.num_layers / 5)))
    Ki = numpy.zeros((inputs.shape[0], int(model.num_layers / 5)))
    Po = numpy.zeros((inputs.shape[0], int(model.num_layers / 5)))
    ...
    for i in range(0, model.num_layers, 5):
        x_z = features[i]
        z_T = x_z[:, int(x_z.shape[1] / 2):x_z.shape[1]].detach().numpy()
        x_T = x_z[:, 0:int(x_z.shape[1] / 2)]

        K = parm['hamil_blocks.%d.mlp.0.weight' % i]
        b = parm['hamil_blocks.%d.mlp.0.bias' % i]
        W = parm['hamil_blocks.%d.vts.weight' % i]

        enegy_kinetic = 1 / 2 * numpy.sum(numpy.multiply(numpy.dot(z_T, numpy.dot(W.T, W)), z_T), 1)
        if activation == 'relu':
            out = model.hamil_blocks[i].mlp(x_T).detach().numpy()
            enegy_potential = numpy.sum(1 / 2 * numpy.power(out, 2), 1)
        elif activation == 'tanh':
            out = model.hamil_blocks[i].mlp[0](x_T).detach().numpy()
            enegy_potential = numpy.sum(numpy.log(abs(numpy.cosh(out))), 1)

        H[:, int(i / 5)] = enegy_kinetic + enegy_potential
        Po[:, int(i / 5)] = enegy_potential
        Ki[:, int(i / 5)] = enegy_kinetic
